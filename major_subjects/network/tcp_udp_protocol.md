...
### 지연 확인 응답
- 작은 윈도를 통지받은 송신 호스트가 그 크기로 데이터를 보내면 네트워크 이용 효율이 떨어지므로 데이터를 수신해도 바로 확인 응답 하지 않는 지연 방법이 고안되었다.
  - (최대 세그먼트 길이 * 2)의 데이터를 수신할 때까지 확인 응답을 하지 않는다.
  - 그렇지 않은 경우에는 확인 응답을 최대 0.5초 지연시킨다.
- 데이터 세그먼트 하나에 확인 응답이 하나일 필요는 없다.
- TCP는 슬라이딩 윈도 제어 방식이므로 확인 응답이 적어도 무관하다.
### Piggyback
...
- UDP-Lite는 UDP의 기능을 확장시킨 프로토콜이다.
- UDP 통신에서 체크섬 오류 발생하면 패킷 전체가 파기되는데 오류가 있는 패킷이더라도 패킷 전체를 파기하지 않고 처리하는 편이 나은 경우가 존재하여 고안되었다.
- UDP-Lite는 UDP와 비슷한 기능을 제공하지만, 체크섬을 계산하는 범위를 애플리케이션이 설정할 수 있다. 이를 통해 오류가 발생하면 안되는 부분에 관해서만 체크섬을 검사하여 그렇지 않은 부분의 오류는 무시하고 데이터를 애플리케이션에 전달할 수 있다.
## SCTP(Stream Control Transmission Protocol)
- SCTP는 데이터 도달성에 신뢰성을 제공하는 트랜스포트 프로토콜이다.
- SCTP는 애플리케이션간의 통신에 작은 메시지를 많이 보낼 때 적합하다.
  - SCTP에서 애플리케이션의 메시지를 'chunk'라고부르며 여러 개의 chunk를 묶어 하나의 패킷을만든다.
- SCTP는 멀티허밍을 지원하여 여러 개의 IP 주소를 설정할 수 있다.
  - 멀티허밍이란 하나의 호스트가 여러 개의 네트워크 인터페이스를 구비하는 것을 의미한다.
- SCTP의 경우 여러개의 IP 주소를 관리하면서 통신할 수 있기 때문에 무선 LAN과 이더넷을 전환해도 통신을 유지할 수 있다. 그러므로 SCTP는 여러 개의 NIC를 갖고 있는 호스트에서 통신의 신뢰성을 높일 수 있다.
- SCTP 주요 특징
  - 메시지 단위 송수신 : SCTP에서 송신 측 애플리케이션이 정한 메시지 크기가 수신 측 애플리케이션에 전달된다.
  - 멀티허밍 지원 : 여러 개의 NIC를 가진 호스트에서 사용할 수 있는 NIC가 바뀌어도 통신을 지속할 수 있다.
  - 여러 개의 스트림 통신 : 여러 개의 커넥션을 확립하여 통신하는 듯한 효과를 하나의 커넥션으로 실현할 수 있다.
  - 메시지 생존 시간 정의 : 생존 시간이 지난 메시지는 재전송하지 않는다.
## DCCP(Datagram Congestion Control Protocol)
- UDP를 사용하는 경우에도 혼잡 제어가 필요하기 때문에 고안되었다.
- DCCP 주요 특징
  - 데이터 도달성에 관한 신뢰성은 없다.
  - 커넥션 지향으로 커넥션의 확립과 끊기 처리가 있고 신뢰성을 보장한다.
  - 네트워크의 혼잡 제어를 할 수 있다.
  - 혼잡 제어를 하기 위해 패킷을 수신한 측은 ACK를 반환한다. 이 확인 응답을 사용하여 재전송이 가능하다.
