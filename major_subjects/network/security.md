...
## 3-1. IPsec과 VPN
### VPN
- 정보의 누출을 막기 위해 인터넷을 이용한 가상의 프라이빗 네트워크를 사용하게 되었는데 이것이 'VPN'이다.
- 인터넷에서는 보안을 높이기 위해 '정보가 노출되어도 해독할 수 없다', '무단 수정된 것을 감지할 수 있다.'라는 방법을 취하는데, 이때 사용하는 것이 '암호화'와 '인증 기술'이고 VPN은 이러한 기술을 이용하여 구축한다.
### IPsec
- VPN을 구축할 때 일반적으로 IPsec을 이용한다.
- IPsec은 IP 헤더의 끝에 '암호 헤더'와 '인증 헤더'를 붙인다. 그리고 헤더 이후의 데이터를 암호화하여 해독할 수 없도록 한다.
- 패킷을 송신할 때 '암호 헤더' 또는 '인증 헤더'를 붙여서, 패킷을 수신할 때 이러한 헤더를 해석하여 데이터를 복원하고 보통의 패킷으로 되돌린다.
- 이러한 처리를 함으로써 암호화된 데이터는 해독할 수 없게 되고, 데이터가 무단 수정되었을 때 판별할 수 있다.

## 3-2. TLS/SSL과 HTTPS
- 웹에서는 TLS/SSL이라는 장치를 통해 HTTP 통신의 암호화를 이룬다.
- TLS/SSL을 사용한 HTTP 통신을 HTTPS라고 한다.
- HTTPS에서는 공통 키 암호화 방식으로 암호화를 처리한다.
- 이 공통 키를 송신할 때에는 공개 키 암호화 방식을 사용한다.
