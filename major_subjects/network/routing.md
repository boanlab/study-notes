...
## OSPF의 기초 지식
- OSPF에서 동일한 링크에 연결되어 있는 라우터를 '인접 라우터'라고 한다.
- 라우터가 1대1로 연결되는 네트워크의 경우 인접 네트워크 간에 경로 정보를 교환한다.
- 여러 개의 라우터가 동일한 링크에 연결된 경우 '지명 라우터'를 정해 지명 라우터를 중심으로 경로 제어 정보를 교환한다.
## OSPF의 동작 개요
- OSPF에서 연결 확인을 하는 프로토콜을 'HELLO 프로토콜'이라 한다.
- LAN의 경우 10초에 한번 HELLO 패킷을 송신한다.
- 4번(40초) 주기로 패킷이 오지 않는 경우 연결이 끊어졌다고 판단한다.
- 연결 상태의 변화가 있는 경우 링크 상태 갱신 패킷을 송신하여 다른 라우터에게 네트워크의 상태 변화를 전달한다.
- 네트워크 LSA와 라우터 LSA가 OSPF에 보내지면, 각 라우터는 네트워크의 구조를 나타내는 링크 상태 데이터베이스를 작성하고 이를 바탕으로 경로 제어표를 작성한다.
- 경로 제어표 작성에는 '다익스트라법'이라는 최단 경로를 구하기 위한 알고리즘을 사용한다.
### 네트워크 LSA
- 링크 상태 갱신 패킷으로 전달하는 정보로, 네트워크에 어떤 라우터가 연결되어 있는지 나타냄 
### 라우터 LSA
- 링크 상태 갱신 패킷으로 전달하는 정보로, 라우터에 어떤 네트워크가 연결되어 있는지 나타냄


# BGP(Border Gateway Protocol)
- BGP는 조직끼리를 연결할 때에 이용하는 프로토콜로, EGP로 분류된다.
## BGP와 AS 번호
- BGP에서는 인터넷 전체를 커버하도록 경로 제어를 해야합니다.
- BG에서 최종적인 경로 제어표는 네트워크 주소와 그 다음에 전송해야 할 라우터로 표현한다. 단, BGP에서 통과하는 AS(경로 제어 도메인)의 수를 바탕으로 경로 제어를 한다.
## BGP는 경로 벡터
- BGP에 의해 경로 제어 정보를 교환하는 라우터를 'BGP 스피커'라고 한다.
- BGP 스피커는 AS 사이에 BGP 정보를 교환하기 위해, 정보를 교환하는 모든 AS와 대등하게 BGP 커넥션을 확립합니다.
- BGP에서 목적하는 네트워크 주소에 패킷을 보낸 경우, 그곳에 도착할 때까지 통과한 AS 번호의 목록을 생성한다.
- 경로 선택에 사용되는 메트릭은 AS이다.


# MPLS(Multi Protocol Label Switching)
- 라벨 스위칭에서는 각각의 IP 패킷에 '라벨'이라는 별도의 값을 설정하여 그 라벨을 바탕으로 전송한다.
## MPLS 네트워크의 동작
- MPLS 네트워크에서 MPLS 기능을 지원하는 라우터를 'LSR'이라 한다.
- 외부 네트워크와 연결된 부분에 있는 엣지 LSR을 'LER'이라 한다.
- LER에서 MPLS 태그를 패킷에 붙이거나 제거합니다.
- 패킷에 라벨을 붙이는 방법
  - 원래 라벨에 해당하는 것을 갖고 있는 데이터 링크인 경우, 거기에 직접 라벨을 매핑한다.
  - 라벨에 해당하는 것을 갖고 있지 않은 데이터 링크의 경우는 새로 '심 헤더'라는 것을 추가하고, 이 심 헤더 안에 라벨을  포함시킨다.
## MPLS의 장점
- 전송 처리의 고속화
- 처리할 데이터량이 줄어듬
- 라벨을 이용하여 가상 경로를 만든 후, 그 위에서 IP와 같은 패킷을 사용한 통신이 가능하다.
