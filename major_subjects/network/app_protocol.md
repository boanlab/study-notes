...
- P2P에서는 각 호스트가 서버, 클라이언트 모두의 기능을 가지고 대등한 관계로 서비스를 서로 제공한다.
## LDAP(Lightweight Directory Access Protocol)
- LDAP는 디렉토리 서비스에 액세스하기 위한 프로토콜이다.
- '디렉토리 서비스'란 네트워크상에 존재하는 다양한 자원에 관해 DB와 같은 정보 제공을 하는 서비스를 칭한다.
- LDAP에서는 디렉토리 트리 구조와 데이터형, 명명 규칙, 디렉토리 트리에 대한 액세스 방법, 보안을 정하고 있다.
- LDAP에 모든 관리 대상 이용자가 등록되어 있다면, 일원적으로 관리되는 LDAP 서버의 정보를 이용 시에 참조함으로써 올바른 이용자인지, 아닌지 판단할 수 있다.
- 따라서 LDAP는 인증 관리나 자원 관리에 사용된다.
