...
- 중간 제곱 함수는 키값을 제곱한 결과값에서 중간에 있는 적당한 비트를 주소로 사용하는 방법

- 제곱한 값의 중간 비트들을 대개 키의 모든 값과 관련이 있음

- 따라서 서로 다른 키값은 서로 다른 중간 제곱 함수값을 가짐
...
**단점**
- 하나의 해시 값에 여러 엔트리가 몰릴 수 있다. 이렇게 되면 배열, 리스트와 같은 선형적인 구조를 갖는 것이나 마찬가지기에 탐색 시 최악의 경우 O(N)의 시간복잡도를 갖게  탐색이 빠르다는 해시 테이블의 장점이 퇴색된다.
